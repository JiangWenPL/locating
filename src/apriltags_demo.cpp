...
public:
  ...
  void config(int cam_id, double fx = 0, double fy = 0, double px = 0, double py = 0, double tag_size = 0)
  {
    if (fx && fy && px && py && tag_size)
    {
      m_deviceId = cam_id;
      m_fx = fx;
      m_fy = fy;
  ...
  void print_detection(AprilTags::TagDetection &detection) const
  {
    ...
    cout << "  distance=" << translation.norm()
         ...
         << ", roll=" << roll
         << "\nrotation: \n"
         << rotation(0, 0) << " " << rotation(0, 1) << " " << rotation(2) << "\n"
  ...
  int april_run(void *np_array)
  {
    ...
    if (detections.size())
    {
      ...
      detection.getRelativeTranslationRotation(m_tagSize, m_fx, m_fy, m_px, m_py,
                                               translation, rotation);
      // result = (double *)malloc(sizeof(double) * 12);
      // printf("In aprile run:\n");
      // printf("trans: %.4f %.4f %.4f\n", translation.data()[0], translation.data()[1], translation.data()[2]);
      // printf("rot: %.4f %.4f %.4f\n", rotation(0), rotation(1), rotation(2));
      rotation.transposeInPlace();
      memcpy(result, rotation.data(), sizeof(double) * 9);
      // memcpy(result + sizeof(double) * 9, translation.data(), sizeof(double) * 3);
      // Memcpy not work and I donot kwno why
      result[9] = translation(0);
      result[10] = translation(1);
...
extern "C"
{
  ...
  static Demo demo;
  int init(int cam_id, double fx, double fy, double px, double py, double tag_size)
  {
    demo.setup();
    if (demo.isVideo())
    {
      cout << "Processing video" << endl;
      demo.config(cam_id, fx, fy, px, py, tag_size);
  ...
  int run(void *result)
  {
    return demo.april_run(result);
  }

  int main()
  {
    init(0, 0, 0, 0, 0, 0);
    double *result = (double *)malloc(sizeof(double) * 12);
    while (1)
    {
      if (run(result))
      {
